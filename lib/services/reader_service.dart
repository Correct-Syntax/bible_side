...
class ReaderService {
  ...
  List<Map<String, dynamic>> getNewPage(BuildContext context, int pageKey, Area area) {
    if (area == Area.primary) {
      return pageFromJson(context, primaryAreaJson, primaryAreaBible, ViewBy.chapter, pageKey);
    } else if (area == Area.secondary) {
      return pageFromJson(context, secondaryAreaJson, secondaryAreaBible, ViewBy.chapter, pageKey); //viewBy
    } else {
      return [];
    }
  }
  ...
  List<Map<String, dynamic>> chapterFromOETJson(
    ...
    // It means that the next verse after the 's1'.
  ...
  List<Map<String, dynamic>> sectionFromOETJson(
    ...
    log(pageKey.toString() + "------fff");

    String theSection = bookSectionNameMapping[bookCode]![pageKey][0];
    log(theSection + ' <<<<-----');

    // Get the section at [pageKey]
